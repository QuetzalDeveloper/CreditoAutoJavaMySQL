...
public class ClienteParcial extends javax.swing.JDialog {
    ...
    public ClienteParcial(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        LookAndFeel();
    }
        ...
        if(paterno.isEmpty()){
            ...
            if (a == 0) {
                try {
                    ...
                    sql = sc.GuardarClienteParcial(paterno, materno, nombre, telefono);
                    con.Conectar(cuenta, password);
                    con.Modificar(sql);
                    con.Cerrar();
                    JOptionPane.showMessageDialog(this, "Cliente almacenado");

                    sql = sc.ObtenerIDCliente(paterno, materno, nombre, telefono);
                    con.Conectar(cuenta, password);
                    res = con.Consulta(sql);
                    while (res.next()) {
                        labelId.setText(res.getInt(1)+"");
                    }
                    con.Cerrar();               
                } catch (Exception e) {
