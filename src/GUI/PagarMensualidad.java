...
public class PagarMensualidad extends javax.swing.JDialog {
    ...
    public PagarMensualidad(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        initComponents();
        LookAndFeel();
    }
            ...
            if (!textCantidad.getText().isEmpty()) {
                if (!textRecibo.getText().isEmpty()) {
                    ...
                    if (plazo > 0) {
                        ...
                        sql = sp.GuardarPago(contrato, num, fecha, cantidad, recibo, plazo, areaComentario.getText());
                        con.Conectar(cuenta, password);
                        con.Modificar(sql);
                        con.Cerrar();
